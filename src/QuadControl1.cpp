...
VehicleCommand QuadControl::GenerateMotorCommands(float collThrustCmd, V3F momentCmd)
{
  ...
  F1 + F2 + F3 + F4 = F                             -       (4)
   ...
   Substituting for l using l = L / (2*sqrt(2)), we get;

   F1 = F/4 + tau_x/(L * sqrt(2)) + tau_y/(L * sqrt(2)) + tau_z / (kappa * 4)
   F2 = F/4 - tau_x/(L * sqrt(2)) + tau_y/(L * sqrt(2)) - tau_z / (kappa * 4)
   F3 = F/4 - tau_x/(L * sqrt(2)) - tau_y/(L * sqrt(2)) + tau_z / (kappa * 4)
   F4 = F/4 + tau_x/(L * sqrt(2)) - tau_y/(L * sqrt(2)) + tau_z / (kappa * 4)
   ...
   Let us code with these values and the above equations

  */
  /////////////////////////////// END STUDENT CODE ////////////////////////////

  float F1 = collThrustCmd/4.f + momentCmd.x/(L * sqrtf(2.)) + momentCmd.y/(L * sqrtf(2.)) - momentCmd.z/(kappa * 4);
  float F2 = collThrustCmd/4.f - momentCmd.x/(L * sqrtf(2.)) + momentCmd.y/(L * sqrtf(2.)) + momentCmd.z/(kappa * 4);
  float F3 = collThrustCmd/4.f - momentCmd.x/(L * sqrtf(2.)) - momentCmd.y/(L * sqrtf(2.)) - momentCmd.z/(kappa * 4);
  float F4 = collThrustCmd/4.f + momentCmd.x/(L * sqrtf(2.)) - momentCmd.y/(L * sqrtf(2.)) + momentCmd.z/(kappa * 4);
